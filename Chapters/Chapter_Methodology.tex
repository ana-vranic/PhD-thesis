...
\textbf{the meaning of the scale free distribution}
...
After estimating the parameters fits we should estimate how good the fit is, calculating the p-value. First we fit the empirical data for the model and calculate the Kolmogorov-Smirnov statistics for this fit. $KS = max|S(x)-P(x)|$, which is maximum distance between the cumulative distributions of data and fitted model. From there we estimate $p-value$. \\

\textbf{Alternative distributions} The p-value suggest that the hypothesis is not rejected. Still, even if data are fitted with power-law, it is possible that some other heavy tailed distribution may be good fit as power-law, or better. Using goodness of fit it is possible to calculate the p-value for the alternative distribution and compare it to the p-value of the power-law. If the p-value for power-law is high, while for alternative distribution low, we can say that the power law distribution is more probable fit. \\

The likelihood ratio test. Where the distribution with higher likelihood under data is better fit. The one can calculate the ratio of likelihoods, or its logarithm $R$, which is positive, or negative depending which distribution is better. In order to make the choice between distributions we need a log likelihood ratio that is sufficently positive or negative, and that its sign is not result of fluctuations. Using the method proposed by Vuong, the p-value is calculated, and if p-value is small p<0.1, the obtained R value sign is reilable and we can tell which model provides the better fit. 
