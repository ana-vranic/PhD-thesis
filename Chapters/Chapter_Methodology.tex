...
The density function has following form
\begin{equation}
...
The stretched exponential distribution is defined as:
\begin{equation}
...
Instead of plotting the probability distribution (i.e. creating a histogram), it is possible to calculate the cumulative distribution, defined as $P(k) = \int_{x}^{\infty} p(x^{'}) dx^{'}$ for cumulative function or as $P(k)=\sum_{k^{'}=1}^{k}p(k^{'})$ for the discrete function. For example the CDF function for power law is, is also power-law function but with exponent $\gamma -1$: $P(k)= k^{-(\gamma-1)}$. Note that for cumulative distribution, it is not necessary to use log-binning.
