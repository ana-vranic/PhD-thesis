...
If $\epsilon_t$ is drawn from the log-normal distribution, then $S_t$ also follows log-normal, as the product of log-normal distributions is again log-normal. Still, the $\epsilon$ distribution does not determine the distribution of the $S_jt$. 
...
the parameter $\beta$ is stretching exponent determining the properties of the function $p(x)$. For $\beta=1$, the function is exponential. For $\beta<1$ it is hard to distinguish the distribution from the power law. We have a compressed exponential function for $\beta>1$, so $x$ varies in the narrow range. 
...
Instead of plotting the probability distribution (i.e. creating a histogram), it is possible to calculate the cumulative distribution. $P(k) = \int_{x}^{\infty} p(x^{'}) dx^{'} = x^{-(\gamma-1)}$. It is also the power-law distribution with exponent $\gamma-1$. Note that for cumulative distribution, it is not necessary to use log-binning.
