...
Another approach for studying the community structure of complex networks, Stochastic block model (SBM), assume that nodes are clustered in the groups, and the relations between nodes depends on the probabilities for groups memberships \cite{lee2019review}. In one group, nodes have similar connectivity patterns. To describe the network G with SBM model, we need to define:
...
For given network $G$, and specified the number of communities $k$ we can use the SBM model to infer the nodes assigments into groups, so we need to choose vector $g$ and SBM matrix $p$ such that the likelihood for generating network $G$ is maximized.
