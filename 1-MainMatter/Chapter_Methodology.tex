...
Another approach for studying the community structure of complex networks, Stochastic block model (SBM), is based on connection probabilities between nodes. It is a generative model which includes existence of communities \cite{lee2019review}. Parameters that describe SBM for network G with N nodes are:
...
In the likelihood equation we iterate over all pairs of nodes, so we separate the product over edges we observe in the network G, and edges that are not present. If we define the number of possible edges between two groups is $n_{rs}$, while $e_{rs}$ is number of observed edges between two groups. As all nodes are considered as independent we can switch the product over nodes with product over groups such that
\begin{equation}
