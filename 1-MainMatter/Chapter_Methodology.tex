...
The simplest network measure is \textbf{node degree}, $k$. The degree of node $i$ gives %TODO is or equal 
 the number of nodes attached %TODO adjacent
...
\subsection{Degree correlations} %TODO Degree-degree correlations.
...
The \textbf{degree-degree correlations} in the network are measured by \textbf{assortativity}. %TODO assortativity index ne assortatiity. I nije jedino meren tako,
...
\subsection{Network paths} %TODO moze samo paths
...
The model interpolates between the regular graph when the probability is $p=0$ and the random graph with $p=1$ when all links are randomly rewired. Short distances and high clustering are present in the network for the critical probabilities. %TODO kolika je verovatnoca
