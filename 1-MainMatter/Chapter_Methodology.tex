...
The clustering can be measured by computing the number of links between neighbors of one node \cite{watts1998collective},
\begin{equation}
c_i=2e_i/(k_i(k_i-1))
\end{equation}
...
Partition is the division of the network into $n_c$ groups, where each node belongs to only one group \cite{barabasi2014network}. 
...
The parameter $\alpha$ controls the linking probability dependence on the nodes' age; if $\alpha=0$, the aging of nodes is disregarded. 
...
When $\alpha$ is negative, aging overcomes the role of preferential attachment, and scale-free properties are lost. For significant negative $\alpha$ network becomes a chain; the youngest nodes are those who get connected. 
