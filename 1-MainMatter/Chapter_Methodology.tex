...
Instead of generating networks, opposite task is to find the nodes assigments in partitions for given network, so we need to infer the probability $P(g|G)$, that partition $g$ is created from network $G$.

\begin{equation}
P(g|G)=\frac{P(G|g)P(g)}{P(G)}
\end{equation}

Finally, we have to maximize log-likelihood function over group membership vector $g$. It can be done numerically. We should notice that the formulation of SBM model does not consider the way to infer the optimal number of groups. Optimizing the likelihood function for different numbers of groups would result in the increase of likelihood, while each node is not assigned to different group. In practice, our found community structures for fixed number of groups, and then likelihood function could be penalized by the number of model parameters. One approach is calculating the Minimum description length (MDL). 

\textbf{MDL.} For the variable that occurs with probability P(x), the necessary ammount of information to describe it is $-log_2P(x)$. The numerator of posterior probability $P(G|g)P(g) = P(A|p, g)P(p, g)=2^{-\Sigma}$, where $\Sigma$ is description length (DL) of the data. It means that if we find the network partition that maximize the posterior distribution, we are also finding the minimum description length (MDL). The MDL consists of two terms: $\Sigma = -log2(p(A|p,g))-log2P(p,g)$. In first part of equation the ammount of information necessary to describe the model decrease with number of groups. The second contribution comes only from the model, and as model becomes more complex, with larger number of groups, this part increases. The optimal solution represents the balance between these two terms in MDL equation.  
