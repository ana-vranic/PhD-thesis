...
Another approach for studying the community structure of complex networks, Stochastic block model (SBM), is based on connection probabilities between nodes. 
%The basic idea of the standard SBM modelis that neighbourhood relations of each node only depend on the probabilities given by the group memberships. The nodes are clustered in the way, so that the neighbours of nodes in a group have a similar patttern as well. First we explain the generative SBM model, and after that give theory for reverse process, the inference of community structure. 
It is a generative model which includes existence of communities \cite{lee2019review}. Parameters that describe SBM for network G with N nodes are:

\begin{itemize}
	\item k: number of groups
	\item group assignment vector, g: $g_i \in\{1,2..k\}$, gives the group index of node $i$.
	\item SBM matrix, $p_{k \times k}$, whose elements $p_{ij}$ are the probabilities that edges between groups $g_i$ and $g_j$ exist. Note that nodes within one group have the same connection probabilities. 
\end{itemize}

The benefit of this model is that we can generate many networks with similar network structure \cite{rosvall2019different}. When model parameters are initialized, the network could be easily generated. For each pair of nodes $i$ and $j$ in network $G$ we draw link if random number $r_{ij}<p_{g_i, g_j}$. 
...
In the likelihood equation we iterate over all pairs of nodes, so we separate the product over edges we observe in the network G, and edges that are not present. If we define the number of possible edges between  groups $r$ and $s$ is 
$n_{rs} = n_rn_s$ if $ r\neq s$, while when $r=s$, $n_{rs} = n_r(n_r+1)/2$  while $e_{rs}=\frac{1}{1+\delta_{rs}}\sum_{i \in r, j\in s} A_{ij}$ is number of observed edges between two groups. Here $A=A_{ij}$ is adjacency matrix As all nodes are considered as independent we can switch the product over nodes with product over groups such that
\begin{equation}
P(G|p, g) = \prod_{(r,s)} p_{g_rg_s}^{e_{rs}}(1-p_{g_rg_s})^{n_{rs}-e_{rs}}
\end{equation}

As it is easier to work with logarithm of likelihood function, after taking logarithm of likelihood function we get following expression that needs to be maximized:
\begin{equation}
L = log(P(G|g, p)) = \sum_{r,s} E_{r,s} ln\frac{E_{rs}}{N_{rs}} + (N_{rs}-E_{rs})ln(\frac{N_{rs}-E_{rs}}{N_{rs}})
\end{equation}

%check
%Instead of generating networks, our task is to find out wich partition b, generated observed network A, assuming that it is done with SBM model. We need probability P(b|A), that partition b is created from network A. $P(b|A)=\frac{P(A|b)P(b)P(b)}{P(A)}$

%MDL
%If descrete variable occurs with probability P(x) the ammount of information necessary to describe it is $-log_2P(x)$. So we may write the numerator of poserior probability as $P(A|b)P(b)=P(A|e, b)P(e,b)=2^{-\Sigma}$, where $\Sigma$ is called description lenght of data. It corresponds to the amount of information necesary to encode the data A together with model parameters e and b. So if we find the network partition that maxiize the posterior distribution, we are finding the one that minimize the description length. As the size of the model increase, with larger number of groups, the ammount of information necessary to describe it will decrease. The ammount of information necessary to describe the model itself will increase as model becomes more complex. So the latter function is penalty, so optimal choice of parameters will lead to balance between these two terms. $\sigma = -log2P(A, e, b)= -log2(p(A|e,b))-log2P(e,b)$

Finally, we have to maximize log-likelihood function over group membership vector $g$. It can be done numerically.

%test
%A challenge of any variant of SBM model is the requirment of the a priori knowledge about he actual number of groups. Simply minimizing the one likelihood function would result in the assigning each node to different community. Most aproaches to this problem consider first to find the optimal community structure for different number of groups and then penalize the calculated likelihood function based on the number of model parameters. 


This simply formulated model has many variants, motivated by specific properties of real data. For example, for networks which are degree heterogeneous, there is degree corrected SBM \cite{karrer2011stochastic}. In some social networks, users can belong to more than one group, and this can be modelled with mixed membership SBM. Other extensions include application to bipartite, weighted network, hierarchical model \cite{funke2019}

Many community detection algorithms define the community as assortative structure. With SBM model, such limitations do not exist and it is possible to directly use the statistical inference for discovering core-periphery structure or even on bipartite networks. 

%test
%One attractive feature of other community detection algorithms is that many automatically choose the number of groups c. The formulation of the SBM and DC-SBM above do not include a way to do this, but there are many ways to add such a behavior to the model. The key step to doing so is to control, somehow, for the additional model flexibility that comes with increasing c—having an extra group significantly in increases the model’s ability to fit the observed network because the The standard solution is regularization, which penal- izes a model’s fit to the data in a way that grows with the model’s flexibility. Hence, more flexible models have to fit the data much better than less flexible models, in order to be worth it. But, just as there are many different ways to detect communities, there are many different ways to regularize a model. 19 The process of choosing one out many models is called model selection or model comparison. (Not to be confused with a goodness-of-fit test, which asks whether a single model is a good fit to the data.) There are many approaches, each of which comes with a set of philosophical assumptions about what makes a better model. The most common is a kind of penalized likelihood function, in which we add to the log-likelihood a penalty function that grows with the “size” of the model. The AIC and BIC are examples of this idea, and MDL is a less well-known example. Another approach is to consider likelihood ratios between smaller and larger models, which can allow one to say that neither model is a better fit than the other; or to integrate the Bayesian posterior distribution over all choices of c. Etc. Which approach a scientist favors often comes down to the scientific culture and history of their field. Inference methods the SBM variants and the corresponding likelihood funtions we need methods to approximate optimal community structure of the selected model. The task of inference algorithm is simply to retrive a good approximation of the greatest element. Markov chain Monte Carlo methods are a class of algotithms used to sample from probability distribution. The widely used variant of this algorithm is Metropolis algorithm. The algorithm suggests the random moves  and accepts any move that improves the function F, and accepts all other moves with probability $exp(-\beta \delta F)$

\textbf{Core-periphery} structure describes a network whose nodes are divided into two community, densely connected core and less connected periphery \cite{csermely2013structure}. If we consider the average probabilities of edges within each group as $p_{11}$ and $p_{22}$, and between groups $p_{12}$, instead of traditionaly assortative or dissasortative structure we can define core-periphery structure $p_{11}> p_{12} > p_{22}$. %In the principle core-periphery structure does not have to be limited to only two groups,  the network can have more cores. 
%and we can define layered, onion, structure. The network can have more cores, that are not directly connected to each other. 
...
where $A_{ij}$ is number of edges between nodes $i$ and $j$. Prior on p is modified for core-periphery model such that $P(p)  \sim I_{0<p_{22}<p_{12}<p_{11}<1}$, while prior on $\theta $ consists of three parts: probability of having $2$ blocks; given the number of layers probability $P(n|2)$ of having groups of sizes ${n_1, n_2}$ and the probability $P(\theta|n)$ of having particular assignments of nodes to blocks. 
