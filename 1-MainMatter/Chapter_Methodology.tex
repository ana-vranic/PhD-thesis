...
$n_{rs} = n_rn_s$ if $ r\neq s$, while when $r=s$, $n_{rs} = n_r(n_r+1)/2$  while $e_{rs}=\frac{1}{1+\delta_rs}\sum A_{ij}$ is number of observed edges between two groups. Here $A=A_{ij}$ is adjacency matrix As all nodes are considered as independent we can switch the product over nodes with product over groups such that
\begin{equation}
...
The likelihood of SBM model increase with number of blocks and model itself does not define optimal number of communities. Inferring minimum description length (MDL) \cite{gallagher2020clarified}
