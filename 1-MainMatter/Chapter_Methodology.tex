...
The complex system can be represented by a complex network $G=(V, E)$, where the elements of a system (atoms, proteins, people) map to a set of $N$ nodes $V=\{1, 2, ..., N\}$. The interactions between elements map to $L$ links between nodes, $E = \{ e_1, e_2... e_L\}$. The \textbf{adjacency matrix} ${A} = N \times N$ has value $1$ if there is a connection between two nodes; otherwise, it is $0$ \cite{boccaletti2006complex}. There are a lot of measures to quantify the structure of the network. This section lists the most important measures, such as degree distribution, correlations, and shortest path measures. We also discuss different structures found in the network, such as core-periphery or community structures.  

\subsection{Degree distribution}

The simplest network measure is \textbf{node degree}, $k$. The degree of node $i$ gives the number of nodes attached to node $i$, $k_i = \sum_j A_{ij}$. The network density is the average degree divided by $N-1$, where $N$ is the number of nodes. It is a relative fraction of nodes in the network. In the case of regular networks, such as grids, each node has an equal degree, meaning that nodes in the network have similar roles. In the general case, the networks have a more complicated structure. If the degree sequence is skewed, we can identify nodes with high-degree (hubs). Removing hubs may partition a connected network into several components. %Finally, if we can test isomorphism between two graphs, the starting point is to try if their degree sequences are the same. If they are not the same, then graphs can not be isomorphic.
...
If the graph nodes are statistically independent, the degree distribution completely determines the properties of a network \cite{dorogovtsev2002evolution}. Here we summarize the forms of degree distributions that are mostly found in the complex network theory:
\begin{itemize}
...
%On the logarithmic scale, the distance between two points is proportional to the logarithm of the distance between two points, meaning that distance on the logarithm scale is the same between points 10 and 100 and 100 and 1000.  
...
The D-measure ranges from $0$ to $1$. The lower D-measure is, the more similar networks are, and structures are isomorphic for D-measure $D = 0$.

%OVDE mozda neka slika d-mere da bude intuitivnije kako radi
%\subsection{Real-world networks}
%Real-world networks share similar properties. The mean distance between nodes is smaller than the number of nodes in the network $l << N$, called small-world phenomena. This cause the fast spread of information or even diseases in complex systems. In small-world networks, the number of vertices grows exponentially with distance; thus, $l$ increases as $log(n)$ or slower. Logarithmic scaling can be proved from various network models; also, it is observed in real-world complex systems. The clustering coefficient in real-world networks is usually high. Real-world networks have one important feature; power-law degree distribution; such networks are called scale-free networks.
...
%The model can fit actual data, which results in finding network communities. For the given network $G$ and the number of groups $k$, the best nodes partition $g$ is found by maximizing the likelihood function. Besides inferring communities, SBM has applications in the prediction of missing links.  % etc. Also, several algorithms for the optimization of the likelihood function are proposed.
...
%The probability that partition $g$ is created from network $G$ is called the model's posterior. We need to infer the probability $P(g|G)$ that partition $g$ is created from network $G$.
%\begin{equation}
%P(g, p|G)=\frac{P(G|g)P(g)}{P(G)}
%\end{equation}

%Finally, we have to maximize the log-likelihood function over group membership vector $g$. It can be done numerically. 

%The Markov chain Monte Carlo (MCMC) methods are a class of algorithms used to sample from probability distributions. These methods generate from a probability distribution over model parameters $\theta$ to estimate it. Starting with an initial state, MCMC simulates transitions of the Markov chain to explore the parameter space and obtain the equation's estimation.  %$I = \frac{1}{S}\sum F(\theta^{i})$.  In the Metropolis-Hasting algorithm, at each time step from given state $\theta$ is generated new proposal and if the acceptance probability $min(1, \frac{\sum_t p_t^ip(s\rightarrow r|t)}{ \sum_t p_t^ip(r \rightarrow s|t) })$, where $p_t^i$ is fraction is a likelihood to move the node $r$ to $s$. 

%In each step algorithm chooses the random node and proposes a move to a different move. Then the change of likelihood function is calculated. If the likelihood function increases under the proposed move is accepted with probability $exp(-\Delta L)$. the Metropolist algorithm is the easiest version of MCMC. The algorithm requires the function F, which is proportional to the likelihood and the number of steps. At each time step, suggested the random moves and accepted if improved the function F. 
...
%Markov chain Monte Carlo methods are a class of algorithms used to sample from the probability distribution. The widely used variant of this algorithm is the Metropolis algorithm. The algorithm suggests the random moves  and accepts any move that improves the function F, and accepts all other moves with probability $exp(-\beta \delta F)$
...
%The likelihood of the SBM model increase with the number of blocks, and the model itself does not define the optimal number of communities. Inferring minimum description length (MDL) \cite{gallagher2020clarified} of the model is one approach to decide which model is more likely.
...
%The BA model postulates the presence of preferential attachment. Yet, we can build models that generate scale-free networks without preferential attachment. The link selection model offers the most straightforward mechanism that produces a scale-free network. At each time step, we add new nodes to the network, select the link randomly, and connect the new node to one of the two nodes at the end. With a higher node degree, it is more probable that the node is located at the end of the chosen link. The more k-degree nodes there are, the more likely the $k$ node is at the end of the selected link. The probability that the node at the end of the randomly chosen link has degree k is $q_k = Ckp_k$. The fact that bias is linear with k indicates that the link selection model builds scale-free networks. 
%Copying model can also generate scale-free networks. In each time step, a new node is added to the network. To decide where it connects, we randomly select node u. Then with probability, $p$ new node links to $u$; otherwise, with chance $1-p$, we randomly choose an outgoing link of node $u$ and link the new node to its target. The likelihood that a new node connects to the degree-k node is $P(k)=\frac{p}{N} + \frac{1-p}{2L}k$, the second part is equivalent in selecting a node to randomly selected link. The popularity of the copying model lies in its relevance in natural systems. It is common in social networks, citation networks or even protein interactions.  in optimization when new nodes balance conflicting criteria as they decide where to connect
