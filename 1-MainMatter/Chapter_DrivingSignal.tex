...
\chapter{Evolving complex network structure dependence on the properties of growth signals} % Main chapter title
%Driving signal
%%TODO "Evolving complex network structure dependence on the properties of growth signals" Mislim da je ovo bolji naslov.
...
we find power-law %scaling of 
degree distribution in the Barabasi-Albert model \cite{barabasi1999}. %%TODO ne scaling nego power-law. I stavi citat na kraju.
Models mainly focus on linking rules and their influence on the topology of complex networks. 

Still, the growth of real systems changes over time. In online social networks, new users join daily, and the users' activity might have bursty nature. We can consider a co-authorship network, where links are created between scientists when they publish a paper. %TODO citati nedostaju
The dynamic of real networks can be complex and highly influenced by nonlinear signals. The growth signal, the number of new nodes in each time step, has cycles and trends. Circadian cycles are directly reflected in growth signals, and we also find long-range correlations and multifractal properties. %TODO citati
...
%%TODO Čekaj, zar ne objašnjavamo vezu između stukture mreže i osobina signala? U biti, ovo što sam ja opisala treba da bude prva rečenica. A onda ova tvoja treba da prati jer je početak opisa šta je sve rađeno u sekciji. 
 
 %TODO ageing aging fix kroz celu tezu
...
Finally, we need to define the new nodes' time series. We focus on the growth of two real systems, the \textbf{TECH} \cite{smiljanic2017associative} community in the Meetup website and on two months of \textbf{MySpace} \cite{suvakov2013} social network. %TODO Formalno gledano imamo i randomizovane signale i Poasonov signal

\subsection{Time-series from real systems} %TODO A zašto ovde ne opišeš i Poasonov signal?
...
%%TODO Ovo su više "Structural differences between evolving complex networks" Ne definišeš ovde D-meru
...
Away from the critical line, the fluctuations do not have strong influence on %strongly influence %TODO ili do not have a strong influence ili do not have influence. Ne može do not hae strongly influence
...
Previous research on temporal networks \cite{holme2012} has shown that edge activation properties impact the complex system's dynamics. Also, different studies indicated the importance of fluctuating signals. %TODO citati
